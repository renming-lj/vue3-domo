...
export class LocalStorage {
     ...
     * @param {*} name 缓存名称
     * @param {*} value 缓存值
     * @param {*} time 储存时间 0为永久
     */
    static set (name:string, value:unknown, time:number = 0):void {
        if (!value) {
    ...
    static get (name:string) {
        ...
        if (data) {
            ...
            if (!cacheData || (cacheData.time !== 0 && cacheData.time < new Date())) {
                localStorage.removeItem(name)
                return null
            }
